...
export function useBatchUpdates ( options = {} ) {
    const { interval = 5000, onProcessStart, onProcessComplete, onProcessError, processingFn, enabled = true } = options;

    const { pendingUpdates, isProcessing, processUpdates, setProcessingInterval, addUpdate, clearQueue } =
        useUpdateQueueStore();
