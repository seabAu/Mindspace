...
const DocumentSchemaView = ( { dataType } ) => {
    const { getSchema, schemas } = useGlobalStore();
    let docSchema = getSchema( dataType );
    return (
        <div
            className={ `w-full h-full border border-dashed border-brand-washedBlue-200` }>
            <h1 className={ `text-2xl font-bold w-auto h-full justify-center items-center text-center align-middle` }>
                { `Schema Detail View` }
            </h1>
            ...
            >
                { dataType && schema && utils.val.isObject( schema ) && (
                ...
                { schemas && utils.val.isObject( schemas ) && (
                    <ul className={ `properties-list flex flex-grow flex-col flex-nowrap justify-center items-stretch w-full h-full px-2 py-2 ` }>
                        <Droplist
                            label={ 'Properties' }
                            data={ schemas }
...
export default DocumentSchemaView;
