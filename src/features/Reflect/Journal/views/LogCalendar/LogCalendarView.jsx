...
export function LogCalendarView ( { items } ) {
    ...
    const [ selectedLogData, setSelectedLogData ] = useState( null ); // For the editing form.
    ...
    const handleDeleteItem = ( id ) => {
        if ( confirm( "Are you sure you want to delete this log?" ) ) {
            ...
            handleDeleteSubmit( id, logsData, setLogsData, 'log' );
            ...
            <div className="flex-1 overflow-auto w-full">
                <div className={ twMerge(
                    ...
                    { calendarDays?.map( ( day, i ) => {
                        const logsForDay = getItemsForDate( day );
                        // console.log( "renderCalendarView :: logsForDay = ", logsForDay );
                                                    ...
                                                    <Tooltip key={ log?._id }>
                                                        <TooltipTrigger>
                                                            <div
                                                                ...
                                                                style={ { backgroundColor: stringAsColor( log?.mood ) } }
                                                                onClick={ ( e ) => handleLogClick( e, log ) }
                                                            />
                                                        </TooltipTrigger>
                                                        <TooltipContent side="bottom" className="p-2 max-w-xs !z-[2000]">
                                                            <div className="space-y-1">
                                                                <div className="font-medium text-xs">{ log?.title }</div>
                                                                { log?.description && (
                                                                    <div className="text-xs text-muted-foreground line-clamp-2">{ log?.description }</div>
                                                                ) }
                                                                ...
                                                                <div className="text-xs flex-col">
                                                                    <span className="capitalize">{ format( log?.date, "MMM d, yyyy" ) }</span>
                                                                    <span className="capitalize">Mood: { log?.mood }</span>
                                                                </div>
                                                                {/* { log?.user && <div className="text-xs">Assigned to: { log?.user.name }</div> } */ }
                                                            </div>
                                                        </TooltipContent>
                                                    </Tooltip>
                                                )
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="flex-1">
                                    { utils.val.isValidArray( itemsForSlot, true ) && itemsForSlot?.map( ( log ) => (
                                        <div
                                            key={ log._id }
                                            className="mb-1 p-1 text-xs rounded"
                                            style={ { backgroundColor: stringAsColor( log.mood ) + "33" } }
                                            // onClick={ ( e ) => {
                                            //     // e.stopPropagation();
                                            //     setSelectedLog( log );
                                            ...
                                            //     setNotesOpen( true );
                                            // } }
                                            onClick={ () => handleOpenLogNotes( log ) }
                                        >
                                            <div className="font-medium">{ log.title }</div>
                                            { log.description && <div className="text-xs opacity-75 line-clamp-1">{ log.description }</div> }
                                        </div>
                                    ) ) }
                                </div>
                            </div>
    ...
    const renderSelectedDayItems = () => {
        ...
        if ( !utils.val.isValidArray( selectedDayLogs, true ) ) {
            return (
                <div className="w-full h-full">
                    <div className="p-4 text-center text-muted-foreground">
                        No log exists for { format( selectedDayDate ?? new Date( Date.now() ), "MMMM d, yyyy" ) }. Create one now?
                    </div>

                    <LogCreate logSchema={ getSchema( 'log' ) } logDate={ selectedDayDate ?? new Date( Date.now() ) } />
                </div>
            );
        }

        if ( utils.val.isDefined( selectedLogData ) ) {
            return (
                <div className="w-full h-full">
                    <div className="p-4 text-center text-muted-foreground">
                        Editing log for { format( selectedLogData?.date ?? new Date( selectedDate ), "MMMM d, yyyy" ) }:
                    </div>

                    <LogDetail
                        logOverride={ selectedLogData }
                        editingMode={ true }
                        onSave={ ( data, dataType = 'log' ) => {
                            // handleEditStart( log, 'log' );
                            handleEditSubmit( data, dataType );
                        } }
                        onBack={ () => {
            ...
            <div className="flex flex-row flex-nowrap items-center justify-around gap-2">
                ...
                {/* { dialogType === 'edit' && (
                    <PlannerDialog
                        ...
                        handleSubmit={ ( data ) => { handleEditSubmit( data, 'log' ); } }
                ...
                {/* { selectedLog && ( <NotesSidebar
                    ...
                    fieldKey={ 'notes' }
                    onCloseOnSave={ false }
                    onUpdateItem={ ( data ) => { handleEditSubmit( data, 'log' ); } }
                /> ) } */}
            </div>
        </div>
    );
